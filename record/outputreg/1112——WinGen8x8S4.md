...
- **8 行行缓冲（Line Buffer × 8）**：保存最近 8 行像素
    
- **写入地址计数器（row/col 计数）**：决定当前像素写到哪一行缓冲的哪个位置
    
- **窗口拼接器（64 路拼接 + 打包寄存）**：当 (row,col) 落在 stride=4 的采样点时，把 8×8 的 64 个值按行优先拼成 `win_bus`，并拉高 `win_valid`