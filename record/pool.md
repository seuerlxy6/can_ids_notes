...
梳理一下，第一层运算relu后直接写回iob，一次给8通道的数据，每个通道数据是32*32，所以可以整齐的放在我的8bankIOB中，第二层运算relu后经过pool，依旧是一次8通道的数据，但一共有32个通道，每个通道是3*3的数据，但是第三层就是fc了，所以pool后的结果就是一个3*3*32=288的一维数据存储在iob中，这个要怎么存呢？我想直接在pool的顶层8通道封装的地方，或者再分一个电路代码，把每个通道输出的9个数据排好，然后把32个通道的数据存好后，展成一维再8个8个写回iob