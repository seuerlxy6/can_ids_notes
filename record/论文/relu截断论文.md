运行设备: cpu
校准集加载完毕: 共 2000 张图片
加载模型权重: ../can_net_cp_0.2_20_pruned.pth
正在基于 FP32 模型 + 权重缩放模拟 进行校准...

================================================================================
真实硬件 Shift 参数表 (考虑了权重和输入的整数缩放)
硬件假设: 输入为 UINT8 (0-255), 权重为 INT8 (-128-127)
================================================================================
###### Layer                | Soft Max   | W_Scale(Qw)  | Hard Max(Est)   | FINAL SHIFT 
--------------------------------------------------------------------------------
conv_layers.0        | 2.82       | 2^6         | 45987           | 8           
conv_layers.2        | 6.26       | 2^6         | 102093          | 9           
fc_layers.0          | 14.43      | 2^7         | 470984          | 11          
 fc_layers.2          | 39.91      | 2^7         | 1302765         | 13          
--------------------------------------------------------------------------------
【说明】
1. Soft Max: 你刚才测出的很小的值 (2.82, 6.26...)
2. W_Scale:  权重的放大倍数 (例如 2^6=64倍)
3. Hard Max: 硬件累加器里真实的数值 (几千甚至几万)
4. FINAL SHIFT: 这才是你要填进 Verilog 的值！


|**参数名称**|**符号**|**数据域**|**作用**|**你的数据示例**|
|---|---|---|---|---|
|**Soft Max**|$A_{float}^{max}$|软件 (FP32)|观测数据的统计分布特征|2.82|
|**W_Scale**|$2^{Q_w}$|转换系数|权重的量化放大倍率|$2^6$ (64)|
|**Hard Max**|$A_{int}^{max}$|硬件 (INT32)|**32-bit 累加器位宽设计的依据**|45,987|
|**FINAL SHIFT**|$S$|硬件控制字|**ReLU 模块的截断控制参数**|8|
